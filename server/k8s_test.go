...
func TestK8sWatcherImpl_handleAddThenUpdate(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			name: "a to b",
			initial: svcAndScenarios{
				svc: ` {"metadata": {"annotations": {"mc-router.wroud.dev/externalServerName": "a.com"}}, "spec":{"clusterIP": "1.1.1.1"}}`,
				scenarios: []scenario{
			...
			update: svcAndScenarios{
				svc: ` {"metadata": {"annotations": {"mc-router.wroud.dev/externalServerName": "b.com"}}, "spec":{"clusterIP": "1.1.1.1"}}`,
				scenarios: []scenario{
		...
		},
		{
			...
			update: svcAndScenarios{
				svc: ` {"metadata": {"annotations": {"mc-router.wroud.dev/externalServerName": "a.com,b.com"}}, "spec":{"clusterIP": "1.1.1.1"}}`,
				scenarios: []scenario{
					{server: "a.com", backend: "1.1.1.1:25565"},
					{server: "b.com", backend: "1.1.1.1:25565"},
				},
		...
		},
		{
			...
			initial: svcAndScenarios{
				svc: ` {"metadata": {"annotations": {"mc-router.wroud.dev/externalServerName": "a.com\nb.com"}}, "spec":{"clusterIP": "1.1.1.1"}}`,
				scenarios: []scenario{
					{server: "a.com", backend: "1.1.1.1:25565"},
					{server: "b.com", backend: "1.1.1.1:25565"},
				},
...
func TestK8s_externalName(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			name: "typeChange",
			initial: svcAndScenarios{
				svc: ` {"metadata": {"annotations": {"mc-router.wroud.dev/externalServerName": "a.com"}}, "spec":{"type":"ExternalName", "externalName": "mc-server.com"}}`,
				scenarios: []scenario{
		...
		},
		{
			...
			initial: svcAndScenarios{
				svc: ` {"metadata": {"annotations": {"mc-router.wroud.dev/externalServerName": "a.com,b.com"}}, "spec":{"type":"ExternalName", "externalName": "mc-server.com"}}`,
				scenarios: []scenario{
					{server: "a.com", backend: "mc-server.com:25565"},
			...
			update: svcAndScenarios{
				svc: ` {"metadata": {"annotations": {"mc-router.wroud.dev/externalServerName": "a.com,b.com"}}, "spec":{"type":"ExternalName", "externalName": "1.1.1.1"}}`,
				scenarios: []scenario{
					{server: "a.com", backend: "1.1.1.1:25565"},
					{server: "b.com", backend: "1.1.1.1:25565"},
				},
...
func TestK8s_proxyServerName(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			svc:  `{"metadata": {"annotations": {"mc-router.wroud.dev/externalServerName": "mc.example.com", "mc-router.wroud.dev/proxyServerName": "velocity-proxy:25577"}}, "spec":{"clusterIP": "10.0.0.5"}}`,
			scenarios: []scenario{
		...
		},
		{
			...
			svc:  `{"metadata": {"annotations": {"mc-router.wroud.dev/externalServerName": "mc.example.com", "mc-router.wroud.dev/proxyServerName": "velocity-proxy"}}, "spec":{"clusterIP": "10.0.0.5"}}`,
			scenarios: []scenario{
		...
		},
		{
			...
			svc:  `{"metadata": {"annotations": {"mc-router.wroud.dev/externalServerName": "mc.example.com"}}, "spec":{"clusterIP": "10.0.0.5"}}`,
			scenarios: []scenario{
...
func TestK8s_proxyServerNameScaleEndpoint(t *testing.T) {
	...
	svc := v1.Service{}
	err := json.Unmarshal([]byte(`{"metadata": {"annotations": {"mc-router.wroud.dev/externalServerName": "mc.example.com", "mc-router.wroud.dev/proxyServerName": "velocity:25577"}}, "spec":{"clusterIP": "10.0.0.5"}}`), &svc)
	require.NoError(t, err)
...
func TestK8s_proxyServerNameUpdate(t *testing.T) {
	...
	// Start with proxy
	initialSvc := v1.Service{}
	err := json.Unmarshal([]byte(`{"metadata": {"annotations": {"mc-router.wroud.dev/externalServerName": "mc.example.com", "mc-router.wroud.dev/proxyServerName": "velocity:25577"}}, "spec":{"clusterIP": "10.0.0.5"}}`), &initialSvc)
	require.NoError(t, err)
	...
	updatedSvc := v1.Service{}
	err = json.Unmarshal([]byte(`{"metadata": {"annotations": {"mc-router.wroud.dev/externalServerName": "mc.example.com"}}, "spec":{"clusterIP": "10.0.0.5"}}`), &updatedSvc)
	require.NoError(t, err)
...
func TestK8s_autoScaleWithoutProxy(t *testing.T) {
	...
	svc := v1.Service{}
	err := json.Unmarshal([]byte(`{"metadata": {"annotations": {"mc-router.wroud.dev/externalServerName": "atm-10.example.com", "mc-router.wroud.dev/autoScaleUp": "true", "mc-router.wroud.dev/autoScaleDown": "true"}}, "spec":{"clusterIP": "10.0.0.10"}}`), &svc)
	require.NoError(t, err)
...
func TestK8s_motdAnnotations(t *testing.T) {
	...
	svc := v1.Service{}
	err := json.Unmarshal([]byte(`{"metadata": {"annotations": {"mc-router.wroud.dev/externalServerName": "mc.example.com", "mc-router.wroud.dev/autoScaleUp": "true", "mc-router.wroud.dev/autoScaleAsleepMOTD": "Server is sleeping", "mc-router.wroud.dev/autoScaleLoadingMOTD": "Server is starting"}}, "spec":{"clusterIP": "10.0.0.5"}}`), &svc)
	require.NoError(t, err)
